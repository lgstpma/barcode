 ...
 * Botón "Imprimir GTI 13" → export_code13.php → cola gtin → GK420t_3x2.
 * NO es items.etiqueta = 13 (zpl_etiqueta_13.php, rotada por IP).
 */
...
/**
 * @param string $codigo2 GTIN-13 (u otro código)
 * @param string $descrip Nombre principal
 * @param string $descrip2 Continuación / peso
 * @param string $descrip3 Nombre completo preferido (si existe)
 * @param int    $unidades Unidades en la caja (texto "N unidades")
 * @param int    $cant     Copias a imprimir (^PQ)
 * @param string $elab_day Fecha elaboración Y-m-d → Lote: DD/MM
 * @param int    $expir    Días de vencimiento → Exp: DD/MM
 */
function build_zpl_etiqueta_gtin($codigo2, $descrip, $descrip2, $unidades, $cant = 1, $elab_day = '', $expir = 0, $descrip3 = '')
{
...
^FO100,158^A0N,28,28^FD' . $gtinDisp . '^FS';
	...
	// QR con la misma información + marca
	$qrPayload = $brand . "\n"
		. $nombre . "\n"
		. 'GTIN: ' . $gtinDisp . "\n"
		. $unidadesLine . "\n"
